...
function ImageRingComponent(
  ...
  useEffect(() => {
    ...
    const offset = new THREE.Vector3()
      ...
      .add(right.clone().multiplyScalar(-100))
    ...
    const start = target.clone().add(right.clone().multiplyScalar(-50));
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      spriteRef.current?.position.copy(
        target.clone().add(right.clone().multiplyScalar(-50))
      );
  ...
  useFrame((state, delta) => {
    // // 1) 먼저 그룹의 실제 회전값 받아옴
    // let actualAngle = groupRef.current?.rotation.y || 0;

    // // 2) forcedCategory가 있으면, minAngle~maxAngle 계산 후 clamp
    // if (forcedCategory) {
    //   console.log("forced category:", forcedCategory)
    //   const cat = categories.find((c) => c.name === forcedCategory);
    //   if (cat) {
    //     const anglePerIndex = -Math.PI / 50;
    //     const minA = anglePerIndex * cat.end;
    //     const maxA = anglePerIndex * cat.start;
    //     actualAngle = THREE.MathUtils.clamp(actualAngle, minA, maxA);
    //     groupRef.current.rotation.y = actualAngle;
    //   }
    // }
    // console.log(controlsRef.current.getAzimuthalAngle());
    /** 1) 부드러운 회전 (lerp) */
    if (isRotating) {
      ...
      //     const minA = anglePerIndex * cat.end;
      //     const maxA = anglePerIndex * cat.start;
      //     cur = THREE.MathUtils.clamp(cur, minA, maxA);
      //   }
      // }
      // groupRef.current.rotation.y = cur;
    }
    ...
    for (let i = 0; i < 100; i++) {
      ...
      // 불투명도도 그대로 유지
      mesh.material.opacity = isInCategory ? 1 : 0.2;
    }
    ...
    if (spriteRef.current && camera) {
      ...
      const offset = new THREE.Vector3()
        ...
        .add(right.clone().multiplyScalar(-100)) // 왼쪽
      ...
      spriteRef.current.position.lerp(
        newTarget.clone().add(right.clone().multiplyScalar(-50)),
        0.1 // 부드러운 이동
      );
